...
const columns = [{ id: 'name', label: 'Name', minWidth: 170 },{ id: 'short_name', label: 'Short Name', minWidth: 170 },{ id: 'description', label: 'Description', minWidth: 170 }];
...
export default function Index() {
    ...
    const submitaddform=()=>{
        if(staff_name.length !=0 ){
            axios.post(process.env.REACT_APP_BACKEND_URL+'stakeholder/',{
                name:staff_name,
    ...
    const submiteditform=()=>{
        ...
        if(modeldata.name.length !=0){
            axios.patch(process.env.REACT_APP_BACKEND_URL+'stakeholder/'+modeldata._id,{
                name:modeldata.name,
    ...
    // --------------------------------delete Staff Type-------------------------------------
    const deleteStakeHolderNature=(id)=>{
        
        axios.delete(process.env.REACT_APP_BACKEND_URL+'stakeholder/'+id).then((response)=>{
            setSubmitted(submitted+1);
        }).catch((err)=>{
            console.log(err)
    ...
    const handleEditDescription = (e) => {
        let model_data={...modeldata};
        model_data.description=e.target.value;
        console.log(model_data.description)
        setModelData(model_data);
    };
            ...
            >
                <Box sx={style}>
                    ...
                    <Grid container spacing={2} sx={{ marginTop: '10px' }}>
                        ...
                        <Grid item xs={12}>
                            <TextField fullWidth  id="outlined-basic" label="Description" variant="outlined" value={description} onKeyPress={handleKeyPress} onChange={handleDescription}/>
                        </Grid>
                                    ...
                                    <>
                                        <TableRow hover role="checkbox" tabIndex={-1} key={row.name}>
                                            ...
                                            <TableCell key={row.name}>
                                                ...
                                                <DeleteIcon sx={{ color: 'red' }} onClick={()=>deleteStakeHolderNature(row._id)}/>
                                            </TableCell>
                                        </TableRow>
                                    </>
                                );
                            ...
                            >
                                <Box sx={style}>
                                    ...
                                    <Grid container spacing={2} sx={{ marginTop: '10px' }}>
                                        ...
                                        <Grid item xs={12}>
                                            <TextField fullWidth id="outlined-basic" label={'Description'} value={modeldata.description} variant="outlined" onChange={handleEditDescription}/>
                                        </Grid>
                                        <Grid item xs={12}>
